...
class PrinterInstaller(tk.Tk):
    def __init__(self):
        ...
        script_dir = r"D:\python_projects\printter"  # Set your script directory path here
        icon_path = os.path.join(script_dir, "print.ico")
    ...
    def on_printer_select(self, event):
        ...
        if selected_index:
            ...
            self.printer_listbox.selection_set(selected_index[0])
    ...
    def update_printer_list(self, printers: List[str] = None):
    ...
    def install_selected_printer(self):
        selected_index = self.printer_listbox.curselection()
        if selected_index:
            selected_printer = self.printer_listbox.get(selected_index[0])
            try:
                startupinfo = STARTUPINFO()
                startupinfo.dwFlags |= STARTF_USESHOWWINDOW
                subprocess.run(
                    ["powershell", "-Command", f"Add-Printer -Name '{selected_printer}'"],
                    check=True,
                    startupinfo=startupinfo,
                )
                messagebox.showinfo("Success", f"Printer '{selected_printer}' installed successfully!")
            except CalledProcessError:
                messagebox.showerror("Error", f"Failed to install printer '{selected_printer}'.")

    def clear_placeholder(self, event):
        if self.search_edit.get() == "Enter Printer Name":
            self.search_edit.delete(0, tk.END)
